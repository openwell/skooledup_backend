...
exports.up = function (knex) {
  return knex.schema.createTable('schools', function (tbl) {
    tbl.increments();
    tbl.string('school_name', 255).notNullable().unique();
    tbl.string('school_name_short', 255);
    tbl.text('school_logo');
    tbl.timestamps(false, true);
  }).createTable('faculties', function (tbl) {
    tbl.increments();
    tbl.string('faculty_name', 255).notNullable();
    tbl.integer('school_id').unsigned().notNullable().references('id').inTable('schools').onDelete('CASCADE');
    tbl.timestamps(false, true);
  }).createTable('departments', function (tbl) {
    tbl.increments();
    tbl.string('department_name', 255).notNullable();
    tbl.integer('faculty_id').unsigned().notNullable().references('id').inTable('faculties').onDelete('CASCADE');
    tbl.timestamps(false, true);
  }).createTable('degrees', function (tbl) {
    tbl.increments();
    tbl.string('degree_name', 255).notNullable();
    tbl.timestamps(false, true);
  }).createTable('courses', function (tbl) {
    tbl.increments();
    tbl.string('course_name', 255).notNullable();
    tbl.integer('department_id').unsigned().notNullable().references('id').inTable('departments').onDelete('CASCADE');
    tbl.integer('degree_id').unsigned().notNullable().references('id').inTable('degrees').onDelete('CASCADE');
...
exports.down = function (knex) {
  return knex.schema.dropTableIfExists('courses').dropTableIfExists('degrees').dropTableIfExists('departments').dropTableIfExists('faculties').dropTableIfExists('schools');
}; // school short name
//# sourceMappingURL=20210506100211_school.js.map